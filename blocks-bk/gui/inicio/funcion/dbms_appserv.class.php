...
class dbms_appserv {
	...
	function __construct($configuracion) {
		if (isset($configuracion['db_dns'])) {$this -> servidor = $configuracion['db_dns'];}
		if (isset($configuracion['db_nombre'])) {$this -> db = $configuracion['db_nombre'];}
		if (isset($configuracion['db_usuario'])) {$this -> usuario = $configuracion['db_usuario'];}
		if (isset($configuracion['db_clave'])) {$this -> clave = $configuracion['db_clave'];}
		if (isset($configuracion['db_sys'])) {$this -> dbsys = $configuracion['db_sys'];}
		...
		switch($this->dbsys) {
			case 'mysql' :
				...
				if ($this -> enlace) {
					...
					if ($base) {*/
                                            return $this -> enlace;/*
					} else {$this -> error = mysql_errno();}*/
				} else {error_log($this -> error = mysqli_connect_errno());
					echo "<script language='JavaScript' type='text/javascript'>
                                                top.location.href='../fuera_servicio.php';
                                              </script>";
					exit ;
				}
		}
	...
	function probar_conexion() {
		...
		if ($this -> enlace == TRUE) {return TRUE;} else {return FALSE;}
	 ...
	 * @access public
	 */
	function registro_db($cadena_sql, $numero) {// echo $cadena_sql." dbms<br>";
		if(!$this->enlace)
		{
			return FALSE;
		}
