...
include '/usr/local/apache/htdocs/urano/blocks/gui/inicio/funcion/config.class.php';
include_once '/usr/local/apache/htdocs/urano/blocks/gui/inicio/funcion/funcionGeneral_appserv.class.php';
include_once '/usr/local/apache/htdocs/urano/blocks/gui/inicio/funcion/sesion.class.php';
include_once '/usr/local/apache/htdocs/urano/blocks/gui/inicio/funcion/accesos.class.php';
include_once '/usr/local/apache/htdocs/urano/blocks/gui/inicio/funcion/expirar_sesiones.class.php';
...
class verifica extends funcionGeneral_appserv
{
    ...
    public function __construct()
    {
        ...
        foreach ($_REQUEST as $key => $value) {
            ...
            switch ($control[$key]) {
                ...
                case 'clave':
                    $this->pwd                 = sha1(md5(strtolower($_REQUEST[$key])));
    ...
    function action()
    {
        ...
        } else { //*realiza la validacion del capcha y destruye la sesion*/
        ...
        if ($valida != $this->usser) {
            ...
            if ($t_us == 'estudiante') {
                ...
                if ($this->configuracion['activar_redireccion_estudiante2'] == 'S') {
                    ...
                    $ingresos2 = $this->configuracion['maximo_sesiones_estudiante2'] + 1;
                }
                if ($this->configuracion['activar_redireccion_estudiante3'] == 'S') {
                    $ingresos3 = $this->leer_archivo($this->NumSesionEst3);
                } else {
                    $ingresos3 = $this->configuracion['maximo_sesiones_estudiante3'] + 1;
                }
    ...
    //inicia la busqueda de sesiones antiguas guardadas
    {
        ...
        if ($this->tipoUser == '51' || $this->tipoUser == '52') {
            ...
            if (strtoupper($this->configuracion['recibir_sesiones_estudiantexfuncionario']) == 'S' && $ingresosEst >= $this->configuracion['maximo_sesiones_estudiante']) { //echo "permite sesiones funcionario";exit;
                /*verifica que no se esten usando el minimo de sesiones de funcionarios, y asignarlas a estudiantes*/
                $ingresosFun = $this->leer_archivo($this->NumSesionFun);
                if ($ingresosFun < $this->configuracion['min_sesiones_estudiantexfuncionario']) {
    ...
    function selecciona_pagina($nivel)
    {
        ...
        break;
        case 20: $this->direccionar('../migracion/appserv/administracion/adm_index.php','');
        break;
        ...
        break;	
        case 123: $this->direccionar('../contratista/contratista.php','');
        break;	
        case 125: $this->direccionar('../voto/voto.php','');
        break;	
        default:
        break;
        }  */
