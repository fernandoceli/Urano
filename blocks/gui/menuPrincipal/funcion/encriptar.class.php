...
class encriptar {
	...
	function codificar_arka($cadena) {
		$token = '0xel0t1l';
		if (function_exists ( 'mcrypt_encrypt' )) {
			$cadena = mcrypt_encrypt ( MCRYPT_RIJNDAEL_256, $token, $cadena, MCRYPT_MODE_ECB ) ;
		} else {
	...
	function codificar_kyron($cadena) {
		$tokenKyron = '0xel0t1lwvXl1FO7';
		if (function_exists ( 'mcrypt_encrypt' )) {
			$cadena = mcrypt_encrypt ( MCRYPT_RIJNDAEL_256, $tokenKyron, $cadena, MCRYPT_MODE_ECB ) ;
		} else {
			$cadena = AesCtr::encrypt ( $cadena, $tokenKyron, 256 ) ;
		}
		$cadena=trim($this->base64url_encode($cadena));
		return $cadena;
	}
